/**
 * Lens optics shared by every glass surface.
 *
 * A lens map is an RGBA image read by an SVG `feDisplacementMap`. Red and green encode how far
 * each pixel bends horizontally and vertically (128 = no bend). The glass is a spherical dome: the
 * slope stays near zero across the center and grows steeply toward the edge, and a smooth
 * error-function window confines the visible bend to the outer `depth` pixels. Every displaced
 * sample points inward, toward the center, so the rim shows a stretched, folded copy of the content
 * inside the glass. Blue carries the specular term: a broad glow plus a highlight band along the
 * edge, both brightest where the rim faces the light. This is the construction Aave describes in
 * "Building Glass for the Web".
 *
 * The map is oversampled: small controls get several map pixels per CSS pixel so the bend stays
 * smooth once the filter stretches it over a high-density backdrop. It has no hard boundary at the
 * shape; the surface is clipped by CSS, and a step in the map would otherwise be smeared into a
 * visible ring by the filter's bilinear sampling. The map covers the shape plus a neutral padding so
 * the SVG filter region can extend past the element. Only the top-left quadrant of the bend is
 * computed; the other quadrants mirror it.
 */
